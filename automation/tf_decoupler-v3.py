...
class TerraformStateDecoupler:
    ...
    def execute_moves(self, csv_file='migration_plan.csv', target_state='new_workspace.tfstate'):
        ...
        # Pre-read the CSV to calculate the total rows for progress tracking
        with open(csv_file, 'r') as in_f:
            reader = csv.DictReader(in_f)
            # Filter out any empty rows just in case
            rows = [row for row in reader if row.get('New_Address', '').strip()]

        total_items = len(rows)
        if total_items == 0:
            print("⚠️ No valid mappings found in the CSV.")
            return
        ...
        for current_idx, row in enumerate(rows, 1):
            ...
            print(f"[{current_idx}/{total_items}] Moving: {old_addr}  ->  {new_addr}")
        ...
        print(f"\n🎉 Done! Successfully moved {mv_count} of {total_items} resources.")
    ...
    def execute_rm(self, csv_file='migration_plan.csv'):
        ...
        with open(csv_file, 'r') as in_f:
            reader = csv.DictReader(in_f)
            rows = [row for row in reader if row.get('Old_Address', '').strip()]

        total_items = len(rows)
        if total_items == 0:
            print("⚠️ No valid resources found in the CSV to remove.")
            return
        ...
        for current_idx, row in enumerate(rows, 1):
            ...
            print(f"[{current_idx}/{total_items}] Removing: {old_addr}")
            cmd = [
                "terraform", "state", "rm",
                f"-backup={os.devnull}", # Updated to route backups to the void
                f"-state={self.state_file}",
                old_addr
            ]
        ...
        print(f"\n🎉 Done! Successfully removed {rm_count} of {total_items} resources from '{self.state_file}'.")
